	...
	class GlitchPass extends THREE.Pass {
		...
		render( renderer, writeBuffer, readBuffer
			...
			if ( this.curF % this.randX == 0 || this.goWild == true ) {
				...
				this.uniforms[ 'amount' ].value = (Math.random() / 30)*.2;
				this.uniforms[ 'angle' ].value = THREE.MathUtils.randFloat( - Math.PI, Math.PI )*.1;
				this.uniforms[ 'seed_x' ].value = THREE.MathUtils.randFloat( - 1, 1 )*.1;
				this.uniforms[ 'seed_y' ].value = THREE.MathUtils.randFloat( - 1, 1 )*.1;
				this.uniforms[ 'distortion_x' ].value = 0;//THREE.MathUtils.randFloat( 0, 1 )*.01;
				this.uniforms[ 'distortion_y' ].value = 0;//THREE.MathUtils.randFloat( 0, 1 )*.01;
				this.curF = 0;
				this.generateTrigger();
